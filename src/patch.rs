//! [`Patch`] and [`PatchOp`] -- the editable overlay over a base
//! buffer.
...
///
/// `offset` and `old_len` are interpreted in the source-buffer
/// coordinate system; applying the splice replaces `source[offset
/// .. offset + old_len]` with `new_bytes`. `old_len == 0` is a pure
/// insert; `new_bytes.is_empty()` is a pure delete; `old_len ==
/// new_bytes.len()` is an in-place write.
...
impl Patch {
    ...
    /// Length of the resulting buffer after splicing into a source of
    /// `source_len` bytes. Doesn't validate ordering or bounds.
    ...
    /// Push a splice without sorting or overlap-checking. Use
    /// [`Patch::splice`] / [`Patch::write`] / [`Patch::insert`] /
    /// [`Patch::delete`] instead unless you're rebuilding the patch
    /// from a known-good source like a deserialised structure.
    ...
    /// Apply this patch, verifying `source` against the recorded
    /// [`SourceMetadata`] (if any) first. Returns a freshly-allocated
    /// buffer; the source is not mutated.
    ...
    /// Apply without verifying source metadata or splice bounds.
    ///
    /// # Safety
    /// The caller must guarantee both:
    /// 1. `source` matches the patch's [`SourceMetadata`] (if any),
    ///    or otherwise really is the buffer this patch was built
    ///    against.
    /// 2. Every op's `offset + old_len <= source.len()`.
    ///
    /// Violating either invariant leads to wrong output bytes for
    /// (1) or out-of-bounds reads of `source` for (2). The function
    /// itself uses checked indexing internally for memory safety,
    /// so the worst direct outcome is a panic; the `unsafe` is the
    /// API's promise that you've done the verification once
    /// out-of-band and don't want to repeat it.
    pub unsafe fn apply_unchecked(&self, source: &[u8]) -> Vec<u8> {
        // Bypass metadata verification. Splice ordering / bounds are
        // still enforced internally because checked slicing is the
        // only way to write the loop without `unsafe` blocks here.
    ...
    /// than [`Patch::apply`] when the result is destined for disk and
    /// doesn't need to live in memory first.
...
#[derive(Debug)]
pub enum BuildError {
    /// A new splice would overlap an op already in the patch.
    /// Coalesce or remove the existing op first.
