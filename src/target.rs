...
impl PatchTarget for std::fs::File {
    ...
    fn splice_at(&mut self, offset: u64, old_len: u64, new_bytes: &[u8]) -> Result<(), Self::Error> {
        ...
        if tail_start > file_len {
            return Err(FileTargetError::OutOfBounds { offset, old_len, file_len });
        }
...
pub enum FileTargetError {
    ...
    OutOfBounds { offset: u64, old_len: u64, file_len: u64 },
    /// `offset + old_len` overflows `u64`.
    SpliceRangeOverflow { offset: u64, old_len: u64 },
}
...
impl fmt::Display for FileTargetError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            FileTargetError::OutOfBounds { offset, old_len, file_len } => write!(
                f,
                "splice at [{offset}, {}) exceeds file length {file_len}",
                offset + old_len
            ),
            FileTargetError::SpliceRangeOverflow { offset, old_len } => {
                write!(f, "splice range offset {offset} + old_len {old_len} overflows u64")
            }
        }
    }
}
...
impl core::error::Error for FileTargetError {
    fn source(&self) -> Option<&(dyn core::error::Error + 'static)> {
        match self {
            ...
            FileTargetError::OutOfBounds { .. } | FileTargetError::SpliceRangeOverflow { .. } => None,
        }
    }
}
